...
var impl = {
	...
	iterate: function()
	{
		...
		if(!this.runs_left) {
			BOOMR.info("No runs left!");
            this.finish();
		}
		else if(this.latency_runs) {
            BOOMR.info("latency runs " +this.latency_runs);
			this.load_img('l', this.latency_runs--, this.lat_loaded);
		}
		else {
            BOOMR.info("runs left" +this.runs_left);
			this.results.push({r:[]});
